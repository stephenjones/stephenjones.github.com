---
layout: post
title:  How I Recovered From Changing Permissions on '/var' directory (on Mac OS X)
base: ../../../../
---

<h1>How I Recovered From Changing Permissions on '/var' directory (on Mac OS X)</h1>

<p><b>Environment:</b> Mac OS - 10.9.3 (Mavericks)</p> 

<p><b>Situation:</b>i I made a (big) error and changed the read permissions on the /var directory... (Don't do this at home!) There are many useful programs under the /var directory, especially in /var/bin - using 'sudo' is how I got into this predicament, and now I had lost read access to the program to get myself out... (as it is located in /var/bin)</p>
...
<p>The second step was to use the now available command line terminal with 'root' access to reverse the permission change I had made on the /var directory. The key to accessing the terminal is covered under: "You will boot into the Repair Utilities screen. On top, in the Menu Bar click the Utilities item then select Terminal."</p>

<p>Using the 'df' command you will see that the partition you have started is different to the regular partition. Reverse the permission change (on /var in my case), and then reboot your PC.</p>
